...
Return strict JSON only.`;
    ...
    const parseRecommendationJson = (rawText: string) => {
    ...
    try {
      ...
      parsed = parseRecommendationJson(response.text || "{}");
    } catch (firstError: any) {
      console.warn("Destination recommendation structured attempt failed, retrying:", firstError?.message || firstError);
      const retryResponse = await ai.models.generateContent({
        model,
        contents: `${prompt}\nIMPORTANT: Return one JSON object with a recommendations array and no markdown fences.`,
        config: { responseMimeType: "application/json" },
      });
      parsed = parseRecommendationJson(retryResponse.text || "{}");
    }
