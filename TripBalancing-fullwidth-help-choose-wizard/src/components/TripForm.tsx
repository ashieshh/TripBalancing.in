...
export default function TripForm({ onSubmit, loading }: TripFormProps) {
  ...
  const getDestinationRecommendations = async () => {
    ...
    try {
      ...
      if (nextRecommendations.length === 0) throw new Error("No destination matches were returned. Please try again.");
      setHelpStep(5);
      window.setTimeout(() => document.getElementById("destination-results")?.scrollIntoView({ behavior: "smooth", block: "start" }), 80);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "Unable to recommend destinations.");
      window.setTimeout(() => errorRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }), 80);
    } finally {
  ...
  const nextHelpStep = () => {
    setError(null);
    if (helpStep === 1 && !origin.trim()) return setError("Please enter your starting city.");
    if (helpStep === 3) {
      if (!tripDatesValid || !travelDays) return setError("Please select your trip dates and duration.");
      if (!recommendBudget && (!budgetVal || Number(budgetVal) <= 0)) return setError("Please enter your total budget.");
    }
    setHelpStep((step) => Math.min(4, step + 1));
    window.setTimeout(() => document.getElementById("help-wizard-top")?.scrollIntoView({ behavior: "smooth", block: "start" }), 40);
  };

  const previousHelpStep = () => {
    setError(null);
    if (helpStep <= 1) {
      setPlanningMode("known_destination");
      setRecommendations([]);
      setDestination("");
      return;
    }
    setHelpStep((step) => Math.max(1, step - 1));
    window.setTimeout(() => document.getElementById("help-wizard-top")?.scrollIntoView({ behavior: "smooth", block: "start" }), 40);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950 sm:p-6">
      {error && (
        <div ref={errorRef} className="flex items-start gap-2.5 rounded-2xl border border-rose-100 bg-rose-50/50 p-4 text-sm text-rose-800 dark:border-rose-900/30 dark:bg-rose-950/10 dark:text-rose-400">
      ...
      {planningMode === "help_choose" && (
        <section id="help-wizard-top" className="rounded-3xl border border-violet-200/70 bg-gradient-to-r from-violet-50 via-white to-teal-50 p-4 dark:border-violet-900/40 dark:from-violet-950/20 dark:via-slate-950 dark:to-teal-950/20 sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-violet-600 dark:text-violet-400">Help Me Choose</p>
              <h3 className="mt-1 text-lg font-black text-slate-900 dark:text-white">Step {helpStep} of 5</h3>
            </div>
            <div className="flex flex-1 items-center gap-2 sm:max-w-xl">
              {[1,2,3,4,5].map((step) => (
                <div key={step} className="flex flex-1 items-center gap-2">
                  <div className={`h-2 flex-1 rounded-full transition-all ${step <= helpStep ? "bg-gradient-to-r from-violet-500 to-teal-500" : "bg-slate-200 dark:bg-slate-800"}`} />
                </div>
              ))}
            </div>
          </div>
          <p className="mt-3 text-sm font-semibold text-slate-600 dark:text-slate-300">
            {helpStep === 1 ? "Tell us where you are starting and how far you want to explore." : helpStep === 2 ? "Tell us who is travelling and the experience you want." : helpStep === 3 ? "Set your dates, travelers and budget." : helpStep === 4 ? "Fine-tune the destination match with your preferences and travel history." : "Choose the destination you like best."}
          </p>
        </section>
      )}

      {(planningMode === "known_destination" || helpStep === 1) && (
      ...
      {(planningMode === "known_destination" || helpStep === 2) && (<>
      ...
      {(planningMode === "known_destination" || helpStep === 3) && (<>
      <section className="space-y-3">
      ...
      {planningMode === "help_choose" && helpStep === 4 && (
      ...
      {planningMode === "help_choose" && helpStep < 5 && (
        <div className="sticky bottom-3 z-20 flex items-center justify-between gap-3 rounded-2xl border border-slate-200/80 bg-white/95 p-3 shadow-xl backdrop-blur dark:border-slate-800 dark:bg-slate-950/95">
          <button type="button" onClick={previousHelpStep} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-3 text-sm font-black text-slate-700 transition hover:bg-slate-50 dark:border-slate-800 dark:text-slate-200 dark:hover:bg-slate-900">
            <ArrowLeft className="h-4 w-4" /> Back
          </button>
          {helpStep < 4 ? (
            <button type="button" onClick={nextHelpStep} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-black text-white transition hover:bg-slate-800 dark:bg-white dark:text-slate-950">
              Next <ArrowRight className="h-4 w-4" />
            </button>
          ) : (
            <button type="button" onClick={getDestinationRecommendations} disabled={recommendationLoading || loading} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-fuchsia-500 to-violet-500 px-5 py-3 text-sm font-black text-white shadow-md disabled:opacity-50">
              {recommendationLoading ? "Finding destinations..." : "✨ Recommend Destinations"}
            </button>
          )}
        </div>
      )}

      {planningMode === "help_choose" && helpStep === 5 && recommendations.length > 0 && (
        <section id="destination-results" className="space-y-3"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Best matches</p><h3 className="text-xl font-black">Select one destination to continue</h3></div><div className="grid gap-4 md:grid-cols-3">{recommendations.map((item, index) => <button key={`${item.destination}-${index}`} type="button" onClick={() => setDestination(item.destination)} className={`rounded-2xl border-2 p-5 text-left transition ${destination === item.destination ? "border-teal-500 bg-teal-50 dark:bg-teal-950/20" : "border-slate-200 dark:border-slate-800"}`}><div className="flex items-center justify-between"><span className="font-black">{index + 1}. {item.destination}</span><span className="rounded-full bg-teal-100 px-2 py-1 text-xs font-black text-teal-700">{item.matchScore}%</span></div><p className="mt-2 text-sm text-slate-600 dark:text-slate-400">{item.whyItFits}</p><p className="mt-3 text-xs font-bold text-slate-500">Estimated: {item.estimatedCostRange}</p><p className="mt-1 text-xs text-slate-500">Best for: {item.bestFor.join(", ")}</p></button>)}</div></section>
      )}
      ...
      {(planningMode === "known_destination" || helpStep === 5) && (
