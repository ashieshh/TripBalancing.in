...
Return strict JSON only.`;
    ...
    let parsed: any = null;
    let lastRecommendationError: any = null;

    // Gemini can occasionally return a transient empty/malformed JSON response.
    // Retry once with a simpler JSON-only configuration before surfacing an error to the user.
    for (let attempt = 0; attempt < 2 && !parsed; attempt += 1) {
      try {
        const response = await ai.models.generateContent({
          model: process.env.GEMINI_MODEL || "gemini-2.5-flash",
          contents: attempt === 0 ? prompt : `${prompt}\nIMPORTANT: Return one JSON object only. No markdown fences, commentary, or prose.`,
          config: attempt === 0 ? {
            responseMimeType: "application/json",
            responseSchema: {
              type: Type.OBJECT,
        ...
        if (Array.isArray(candidate?.recommendations) && candidate.recommendations.length > 0) parsed = candidate;
        else throw new Error("AI returned an empty destination list.");
      } catch (attemptError: any) {
        lastRecommendationError = attemptError;
        console.warn(`Destination recommendation attempt ${attempt + 1} failed:`, attemptError?.message || attemptError);
      }
    }

    if (!parsed) {
      console.error("Destination recommendation exhausted retries:", lastRecommendationError?.message || lastRecommendationError);
      return res.status(502).json({ error: "Destination recommendations are temporarily unavailable. Your answers are saved — please try again." });
    }
    ...
    return res.status(500).json({ error: "Unable to recommend destinations right now. Your answers are saved — please try again." });
  }
});
