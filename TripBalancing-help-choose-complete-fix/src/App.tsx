  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <main className="max-w-7xl min-[1920px]:max-w-[1800px] min-[2560px]:max-w-[2400px] mx-auto px-4 sm:px-6 py-8">
        ...
        {/* Error Notification */}
        {apiError && (
          <div ref={apiErrorRef} tabIndex={-1} role="alert" aria-live="assertive" className="scroll-mt-28 mb-8 rounded-3xl border border-rose-300 bg-rose-50/80 p-4 text-rose-800 outline-none ring-4 ring-rose-400/10 shadow-lg dark:border-rose-900/50 dark:bg-rose-950/20 dark:text-rose-300">
            <div className="flex items-start gap-3">
              <AlertCircle className="mt-0.5 h-5 w-5 flex-shrink-0 text-rose-500" />
              <div className="flex-1 space-y-1">
                <h4 className="font-black">We couldn't generate your trip</h4>
                <p className="text-sm leading-relaxed">{apiError}</p>
                <p className="text-xs text-rose-600/80 dark:text-rose-300/70">Your trip details are still saved in the form below.</p>
              </div>
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              {lastGenerationInput && <button type="button" onClick={() => handleGenerateItinerary(lastGenerationInput)} className="rounded-xl bg-rose-600 px-4 py-2 text-xs font-black text-white">Try Again</button>}
              <button type="button" onClick={() => { setApiError(null); window.setTimeout(() => document.querySelector('form')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50); }} className="rounded-xl border border-rose-300 px-4 py-2 text-xs font-black dark:border-rose-800">Back to form</button>
            </div>
          </div>
        )}
