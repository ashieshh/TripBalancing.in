  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const next = raw.filter((item: LocationSuggestion) => {
          const key = `${String(item.name || "").trim().toLowerCase()}|${String(item.admin1 || "").trim().toLowerCase()}|${String(item.country || "").trim().toLowerCase()}`;
          if (!item?.canonicalName || seen.has(key)) return false;
          seen.add(key);
          return true;
        }).slice(0, 5);
        setSuggestions(next);
  ...
  return (
    <div ref={wrapperRef} className="relative space-y-1.5">
      ...
      {open && !confirmed && value.trim().length >= 2 && (
        <div className="absolute z-50 mt-1 max-h-[320px] w-full overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950">
