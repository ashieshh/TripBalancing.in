...
export default function Dashboard({
  ...
  const tabs: { id: DashboardTab; label: string; icon: any }[] = [
    { id: "home", label: "Recent Trips", icon: Home },
        ...
        <div className="space-y-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <div className="mb-2 inline-flex items-center gap-1.5 rounded-full bg-teal-500/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-teal-600 dark:text-teal-400">
                <Clock3 className="h-3 w-3" /> Recent journeys
              </div>
              <h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Continue where you left off</h2>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Only your 3 most recent trips appear here, so the home page always stays compact.</p>
            </div>
            {trips.length > 3 && (
              <button
                type="button"
                onClick={() => setActiveTab("trips")}
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-black text-slate-700 shadow-sm transition hover:border-teal-500/30 hover:text-teal-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 dark:hover:text-teal-400"
              >
                View All {trips.length} Trips <ArrowRight className="h-3.5 w-3.5" />
              </button>
            )}
          </div>
          ...
          {recentTrips.length === 0 ? (
            <div className="rounded-[26px] border border-dashed border-slate-300 bg-white px-6 py-12 text-center dark:border-slate-800 dark:bg-slate-950">
              ...
              <h3 className="mt-3 text-base font-black text-slate-900 dark:text-white">Your first trip will appear here</h3>
              <p className="mt-1 text-sm text-slate-500">Use the planner above to create your first itinerary.</p>
            </div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {recentTrips.map((trip) => {
                ...
                return (
                  <article key={trip.id} className="overflow-hidden rounded-[24px] border border-slate-200/80 bg-white shadow-sm transition hover:-translate-y-1 hover:border-teal-500/30 hover:shadow-lg dark:border-slate-800 dark:bg-slate-950">
                    ...
                    <div className="p-5">
                      ...
                      <h3 className="mt-3 truncate text-xl font-black tracking-tight text-slate-900 dark:text-white">{trip.destination}</h3>
                      <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
                        ...
                        <span className="inline-flex items-center gap-1.5"><Users className="h-3.5 w-3.5" />{trip.travelers}</span>
                      </div>
                      <div className="mt-4 flex items-center justify-between rounded-2xl bg-slate-50 px-3.5 py-3 dark:bg-slate-900/60">
                        <div>
                          <div className="text-[9px] font-black uppercase tracking-wider text-slate-400">Planned budget</div>
                          <div className="mt-0.5 text-sm font-black text-slate-800 dark:text-white">{trip.budgetAmount || "—"}</div>
                        </div>
                        <Briefcase className="h-4 w-4 text-teal-500" />
                      </div>
                      <button onClick={() => onSelectTrip(trip)} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-3 text-xs font-black text-white transition hover:bg-teal-600 dark:bg-white dark:text-slate-950 dark:hover:bg-teal-400">
          ...
          {trips.length > 0 && trips.length <= 3 && (
            <div className="flex justify-end">
              <button onClick={() => setActiveTab("trips")} className="inline-flex items-center gap-1.5 text-xs font-black text-teal-600 hover:underline dark:text-teal-400">Manage My Trips <ArrowRight className="h-3.5 w-3.5" /></button>
            </div>
          )}
        </div>
      )}
